...
class CommodityPaperRuntime:
    ...
    def on_quote(self, secid: int, tag: str, ltp: float, depth) -> None:
        ...
        if now - self.last_score_log_ts[symbol] >= self.settings.heartbeat_sec:
            ...
            logger.info(
                "COMMODITY_PERCENT_STATE | symbol=%s | contract=%s | ltp=%.2f | score=%.1f | state=%.1f | mode=%.0f | entry_ready=%.0f | breakout=%.0f | reclaim=%.0f | entry_q=%.1f | hold_q=%.1f | exit_p=%.1f | trend=%.1f | orderflow=%.1f | liquidity=%.1f | trap=%.1f | opp=%.1f | risk=%.1f | scalp=%.1f | swing=%.1f | regime=%.0f | exit_plan=%.0f | ret5=%.3f%% | ret30=%.3f%% | ret120=%.3f%% | vwap=%.3f%% | spread=%.3f%% | action=%s | reason=%s",
                symbol,
                instrument["trading_symbol"],
                float(ltp),
                signal.score,
                signal.features.get("market_state_code", 0.0),
                signal.features.get("commodity_mode_code", 0.0),
                signal.features.get("long_entry_ready", 0.0),
                signal.features.get("breakout_watch", 0.0),
                signal.features.get("pullback_reclaim_ready", 0.0),
                signal.features.get("entry_quality_score", 0.0),
                signal.features.get("hold_quality_score", 0.0),
                signal.features.get("exit_pressure_score", 0.0),
                signal.features.get("trend_score", 0.0),
                signal.features.get("orderflow_score", 0.0),
                signal.features.get("liquidity_score", 0.0),
                signal.features.get("trap_risk_score", 0.0),
        ...
        if now - self.last_audit_log_ts[symbol] >= self.settings.tick_audit_sec:
            ...
            logger.info(
                ...
                signal.features.get("trend_score", 0.0),
                signal.features.get("breakout_score", 0.0),
                signal.features.get("pullback_reclaim_score", 0.0),
        ...
        if signal.action == "ENTRY" and self._entry_allowed(symbol, now):
            if self.portfolio.enter(
                ...
                logger.info(
                    "COMMODITY_ENTRY_COMMITTED | symbol=%s | contract=%s | secid=%s | qty=%s | entry=%.2f | score=%.1f | entry_q=%.1f | hold_q=%.1f | exit_p=%.1f | trap=%.1f | opp=%.1f | risk=%.1f | mode=%.0f | exit_plan=%.0f | reason=%s",
                    symbol,
                    instrument["trading_symbol"],
                    secid,
                    position.qty,
