...
from dhan_engine.domain.trade_lifecycle.trade_state import TradeState
from dhan_engine.domain.trade_lifecycle.entry_acceptance_analyzer import EntryAcceptanceAnalyzer
from dhan_engine.domain.trade_lifecycle.failure_analyzer import FailureAnalyzer
from dhan_engine.domain.trade_lifecycle.multi_tf_bias_filter import MultiTimeframeBiasFilter
from dhan_engine.domain.trade_lifecycle.momentum_phase_manager import MomentumPhaseManager
...
class OptionsMomentumEngine:
    ...
    def register_trade(self, secid: int, trade: dict) -> None:
        trade_payload = dict(trade)
        self.active_trade[secid] = trade_payload
        entry = float(trade_payload.get("entry", 0.0) or 0.0)
        ts = float(trade_payload.get("ts", time.time()))
        self.trade_state[secid] = TradeState(entry, ts)

    def clear_trade(self, secid: int, reason: str | None = None) -> None:
    ...
    def _evaluate(self, secid: int) -> str:
        ...
        if secid in self.active_trade:
            ...
            if state is None:
                state = TradeState(entry, float(t.get("ts", last["ts"])))
                self.trade_state[secid] = state
            if state:
